...
directory = "his/"  # 원래 "raw_data/" 하단 폴더에 작업 시
outfile_name = "RMS DN History.csv"
...
for filename in files:  # 반복수행
    ...
    for line in file:  # 한줄씩 차례대로 위에서 끝까지 불러오고, 마지막 줄 후 종료
        if "F" in line:  # 내부에 F가 없으면 수행하지 않을 것
            line = line.replace(",", ".")
    ...
    out_file.write("번호,RMS ID,현장명,대수,전화번호,비고,,횟수,,시작시간,종료시간")

    for group in chunker(contents, 11):
        new_line = ", ".join(group)  # join함수로 컨텐트 사이에 ,삽입
